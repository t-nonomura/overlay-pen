...
- Passive Annotation では描画の見え方を維持する実装にしている
...
Android 12 以降、touch pass-through は overlay の不透明度制約を受ける可能性がある。  
現在は描画の見え方を維持する実装を採用し、実機差分を継続確認する。
...
- `最小化` しても描画の見え方は変えない
- `最小化` では compact な `Tools` チップへ移行する
- セッション終了時に描画を完全破棄する
...
- 描画を残したまま下位アプリへ戻ること
  - 実機差分の継続確認が必要
  - touch pass-through は OEM 差分の影響を受けうる
